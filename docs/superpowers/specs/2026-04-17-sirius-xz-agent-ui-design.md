...
The shell also includes a persistent status strip above the tabs so the runtime mode and sync state stay visible while users switch areas.
...
The first iteration does not need a router. Tab switching is enough for this scope and keeps the interaction model close to a product console rather than a multi-page app.
...
- `TabBar`
  - main section switcher with per-section counters
- `DocumentList`
...
- the overview, chat, knowledge, and API sections all render
